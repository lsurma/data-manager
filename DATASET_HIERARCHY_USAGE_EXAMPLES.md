...
@code {
    ...
    private async Task LoadDataSetHierarchy(Guid rootId)
    {
        ...
        hierarchyDataSets = result.DataSets;
...
{
    // Create test data
    var dataSetA = new DataSet { Id = Guid.NewGuid(), Name = "Dataset A" };
    var dataSetB = new DataSet { Id = Guid.NewGuid(), Name = "Dataset B" };
    var globalData = new DataSet { Id = Guid.NewGuid(), Name = "GlobalData" };
    var final = new DataSet { Id = Guid.NewGuid(), Name = "Final" };

    await _context.DataSets.AddRangeAsync(dataSetA, dataSetB, globalData, final);

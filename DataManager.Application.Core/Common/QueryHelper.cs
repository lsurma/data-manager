    ...
    where TPrimaryKey : notnull
{
    ...
    /// <summary>
    /// Gets the local query (from change tracker) for this entity.
    /// Override this property in derived classes to provide the Local queryable from your DbContext.
    /// By default, returns the DefaultQuery (which will query the database).
    /// </summary>
    protected virtual IQueryable<TEntity> LocalQuery => DefaultQuery;
    ...
    /// <summary>
    /// Gets the local query from change tracker
    /// </summary>
    protected IQueryable<TEntity> GetLocalQuery() => LocalQuery;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (options is { UseLocalView: true })
        {

...
/// <summary>
/// Query service for TranslationsSet entities with authorization filtering
/// </summary>
public class TranslationsSetsQueryService : QueryService<TranslationsSet, Guid>
{
    private readonly DataManagerDbContext _context;
    private readonly IAuthorizationService _authorizationService;

    public TranslationsSetsQueryService(
    ...
    protected override IQueryable<TranslationsSet> DefaultQuery => _context.TranslationsSets;
    ...
    /// </summary>
    public async Task<IQueryable<TranslationsSet>> ApplyAuthorizationAsync(
        IQueryable<TranslationsSet> query,
    ...
    /// </summary>
    public override async Task<IQueryable<TranslationsSet>> PrepareQueryAsync(
        IQueryable<TranslationsSet>? query = null,
        QueryOptions<TranslationsSet, Guid>? options = null,
        CancellationToken cancellationToken = default)
    {
    ...
    public class Options : QueryOptions<TranslationsSet, Guid, TranslationsSet>
    {
        public Options()
        {
            AsNoTracking = true;
    ...
    public async Task<List<Guid>> GetDataSetHierarchyIdsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var (hierarchyIds, _) = await GetDataSetHierarchyInternalAsync(rootTranslationsSetId, cancellationToken);
        return hierarchyIds;
    }
    ...
    public async Task<List<TranslationsSet>> GetTranslationsSetHierarchyAsync(
        Guid rootTranslationsSetId,
        CancellationToken cancellationToken = default)
    {
        var (hierarchyIds, dataSetLookup) = await GetDataSetHierarchyInternalAsync(rootTranslationsSetId, cancellationToken);

        // Return datasets in the same order as hierarchy IDs
        var result = new List<TranslationsSet>();
        foreach (var id in hierarchyIds)
        {
            if (dataSetLookup.TryGetValue(id, out var translationsSet))
            {
    ...
    /// </summary>
    private async Task<(List<Guid> hierarchyIds, Dictionary<Guid, TranslationsSet> dataSetLookup)> GetDataSetHierarchyInternalAsync(
        Guid rootTranslationsSetId,
        CancellationToken cancellationToken = default)
    {
        ...
        var query = await ApplyAuthorizationAsync(_context.TranslationsSets, cancellationToken);
        
        var allTranslationsSets = await query
